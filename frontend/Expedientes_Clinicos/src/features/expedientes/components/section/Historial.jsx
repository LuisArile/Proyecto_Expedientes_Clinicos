      ...
      <TabsContent value="historial" className="mt-0 space-y-4">
        ...
        <div className="space-y-3">
          {eventos.length > 0 ? (
                  ...
                  <div className="flex-1 pb-4">
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-start justify-between mb-2">
                        <p className="font-semibold text-gray-900"> {esConsulta ? "Consulta Médica" : "Registro Preclínico"}</p>
                        <p className="text-sm text-gray-600">{fechaFormateada}</p>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-700">
                        <User className="h-3 w-3" /><span>{esConsulta ? `Dr. ${personalNombre}` : `Enf. ${personalNombre}`}</span>
                      </div>
                      {esConsulta && diagnosticoTexto && (
                        <div className="mt-2">
                          <p className="text-xs font-bold text-gray-500">Diagnóstico: {diagnosticoTexto}</p>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <p className="text-center text-gray-500 py-10">No hay actividad registrada en el historial.</p>
          )}
        </div>
      </TabsContent>
    </CardContent>
